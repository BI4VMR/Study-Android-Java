...
public class SeekBarWithIndicator extends AppCompatSeekBar {
    ...
    public SeekBarWithIndicator(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        // thumb偏移量
        float thumbOffset = ((float) mThumbWidth - (float) mProgressTextRect.width()) / 2 - mThumbWidth * progressRatio;
        ...
        if (mIndicatorSeekBarChangeListener != null) {
            float indicatorOffset = getWidth() * progressRatio - ((float) mIndicatorWidth - (float) mThumbWidth) / 2 - mThumbWidth * progressRatio;
